
package com.pi4j.example.PN532;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import com.pi4j.context.Context;
import com.pi4j.io.i2c.I2C;
import com.pi4j.io.i2c.I2CConfig;
import com.pi4j.io.i2c.I2CProvider;
// import com.pi4j.io.gpio.digital;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class PN532I2C implements IPN532Interface {

    private static final Logger LOG = LoggerFactory.getLogger(PN532I2C.class);

    private I2C i2c;
    boolean debug = false;
    boolean debugReads = false;

    private byte command;

    private Context context;
    private int address;
    private int bus;
    private static final int DEFAULT_ADDRESS = 0x24;
    private static final int I2C_BUS = 1;

    public PN532I2C(Context pi4j) {
        this(pi4j,I2C_BUS,DEFAULT_ADDRESS);
    }

    public PN532I2C(Context pi4j,int bus, int address) {
        this.context = pi4j;
        this.bus = bus;
        this.address = address;
    }

    @Override
    public void begin() {
        I2CProvider i2CProvider = this.context.provider("pigpio-i2c");
        I2CConfig i2cConfig = I2C.newConfigBuilder(this.context).id("PN532").bus(this.bus).device(this.address).build();
        i2c = i2CProvider.create(i2cConfig);
        LOG.info("PN532 Connected to i2c bus={} address={}. OK.", this.bus, this.address);
    }

    @Override
    public void wakeup() {

    }

    @Override
    public CommandStatus writeCommand(byte[] header, byte[] body) throws InterruptedException {
        LOG.info("pn532i2c.writeCommand(header:" + getByteString(header) + ", body: " + getByteString(body) + ")");

        List<Byte> toSend = new ArrayList<Byte>();

        command = header[0];

        toSend.add(PN532_PREAMBLE);
        toSend.add(PN532_STARTCODE1);
        toSend.add(PN532_STARTCODE2);

        byte cmd_len = (byte) header.length;
        cmd_len += (byte) body.length;
        cmd_len++;
        byte cmdlen_1 = (byte) (~cmd_len + 1);

        toSend.add(cmd_len);
        toSend.add(cmdlen_1);

        toSend.add(PN532_HOSTTOPN532);

        byte sum = PN532_HOSTTOPN532;

        for (int i = 0; i < header.length; i++) {
            toSend.add(header[i]);
            sum += header[i];
        }

        for (int i = 0; i < body.length; i++) {
            toSend.add(body[i]);
            sum += body[i];
        }

        byte checksum = (byte) (~sum + 1);
        toSend.add(checksum);
        toSend.add(PN532_POSTAMBLE);
        byte[] bytesToSend = new byte[toSend.size()];
        for (int i = 0; i < bytesToSend.length; i++) {
            bytesToSend[i] = toSend.get(i);
        }

        LOG.info("pn532i2c.writeCommand sending " + getByteString(bytesToSend));
        if (i2c.write(bytesToSend, 0, bytesToSend.length)==0)
        {
            LOG.info("pn532i2c.writeCommand exception occured");
            return CommandStatus.INVALID_ACK;
        }

        LOG.info("pn532i2c.writeCommand transferring to waitForAck())");
        return waitForAck(5000);

    }

    private CommandStatus waitForAck(int timeout) throws InterruptedException {
        LOG.info("pn532i2c.waitForAck()");

        byte ackbuff[] = new byte[7];
        byte PN532_ACK[] = new byte[]{0, 0, (byte) 0xFF, 0, (byte) 0xFF, 0};

        int timer = 0;
        String message = "";
        while (true) {

            int read = i2c.read(ackbuff, 0, 7);
            if (debugReads && read > 0) {
                LOG.info("pn532i2c.waitForAck Read " + read + " bytes.");
            }
            
            if ((ackbuff[0] & 1) > 0) {
                break;
            }

            if (timeout != 0) {
                timer += 10;
                if (timer > timeout) {
                    LOG.info("pn532i2c.waitForAck timeout occured: " + message);
                    return CommandStatus.TIMEOUT;
                }
            }
            Thread.sleep(10);
        }

        for (int i = 1; i < ackbuff.length; i++) {
            if (ackbuff[i] != PN532_ACK[i - 1]) {
                LOG.info("pn532i2c.waitForAck Invalid Ack.");
                return CommandStatus.INVALID_ACK;
            }
        }

        LOG.info("pn532i2c.waitForAck OK");
        return CommandStatus.OK;

    }

    @Override
    public CommandStatus writeCommand(byte[] header) throws InterruptedException {
        return writeCommand(header, new byte[0]);
    }

    @Override
    public int readResponse(byte[] buffer, int expectedLength, int timeout) throws InterruptedException {
        LOG.info("pn532i2c.readResponse");

        byte response[] = new byte[expectedLength + 2];

        int timer = 0;

        while (true) {

            int read = i2c.read(response, 0, expectedLength + 2);
            if (debugReads && read > 0) {
                LOG.info("pn532i2c.waitForAck Read " + read + " bytes.");
            }
            

            if ((response[0] & 1) > 0) {
                break;
            }

            if (timeout != 0) {
                timer += 10;
                if (timer > timeout) {
                    LOG.info("pn532i2c.readResponse timeout occured.");
                    return -1;
                }
            }

            Thread.sleep(10);
        }

        int ind = 1;

        if (PN532_PREAMBLE != response[ind++] || PN532_STARTCODE1 != response[ind++] || PN532_STARTCODE2 != response[ind++]) {
            LOG.info("pn532i2c.readResponse bad starting bytes found");
            return -1;
        }

        byte length = response[ind++];
        byte com_length = length;
        com_length += response[ind++];
        if (com_length != 0) {
            LOG.info("pn532i2c.readResponse bad length checksum");
            return -1;
        }

        byte cmd = 1;
        cmd += command;

        if (PN532_PN532TOHOST != response[ind++] || (cmd) != response[ind++]) {
            LOG.info("pn532i2c.readResponse bad command check.");
            return -1;
        }

        length -= 2;
        if (length > expectedLength) {
            LOG.info("pn532i2c.readResponse not enough space");
            return -1;
        }

        byte sum = PN532_PN532TOHOST;
        sum += cmd;

        for (int i = 0; i < length; i++) {
            buffer[i] = response[ind++];
            sum += buffer[i];
        }

        byte checksum = response[ind++];
        checksum += sum;
        if (0 != checksum) {
            LOG.info("pn532i2c.readResponse bad checksum");
            return -1;
        }

        return length;
    }

    @Override
    public int readResponse(byte[] buffer, int expectedLength) throws InterruptedException {
        return readResponse(buffer, expectedLength, 1000);
    }

    private String getByteString(byte[] arr) {
        String output = "[";

        if (arr != null) {
            for (int i = 0; i < arr.length; i++) {
                output += Integer.toHexString(arr[i]) + " ";
            }
        }
        return output.trim() + "]";
    }
  
}
